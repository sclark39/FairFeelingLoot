...
const UGenericGraphNode* USwitchLTNode::PickChild(FLootTable &LootTable, const FEntropyState &State) const
{
	FName *NameParam = LootTable.NameParams.Find(ParamName);
	const UGenericGraphNode *DefaultNode = nullptr;
	if (NameParam)
	{
...
TSubclassOf<UGenericGraphEdge> USwitchLTNode::GetEdgeType() const
{
