...
ULTForceDirectedLayoutStrategy::ULTForceDirectedLayoutStrategy()
{
	bRandomInit = false;
	CoolDownRate = 10;
...
void ULTForceDirectedLayoutStrategy::Layout(UEdGraph* _EdGraph)
{
...
FBox2D ULTForceDirectedLayoutStrategy::LayoutOneTree(ULTGenericGraphNode* RootNode, const FBox2D& PreTreeBound)
{
	...
	for (int32 IterrationNum = 0; IterrationNum < MaxIteration; ++IterrationNum)
	{
		...
		while (CurrLevelNodes.Num() != 0)
		{
			for (int32 i = 0; i < CurrLevelNodes.Num(); ++i)
			{
				...
				UEdNode_LTGenericGraphNode* EdNode_ParentNode = EdGraph->NodeMap[Node];

				for (int32 j = 0; j < Node->ChildrenNodes.Num(); ++j)
				{
					NextLevelNodes.Add(Node->ChildrenNodes[j]);

					UEdNode_LTGenericGraphNode* EdNode_ChildNode = EdGraph->NodeMap[Node->ChildrenNodes[j]];
