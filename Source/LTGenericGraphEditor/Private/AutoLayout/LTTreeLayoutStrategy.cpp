...
ULTTreeLayoutStrategy::ULTTreeLayoutStrategy()
{
}
...
void ULTTreeLayoutStrategy::Layout(UEdGraph* _EdGraph)
{
	EdGraph = Cast<UEdGraph_LTGenericGraph>(_EdGraph);
	check(EdGraph != nullptr);

	EdGraph->RebuildLTGenericGraph();
	Graph = EdGraph->GetLTGenericGraph();
	check(Graph != nullptr);
	...
	{
		ULTGenericGraphNode* RootNode = Graph->RootNodes[i];
...
void ULTTreeLayoutStrategy::InitPass(ULTGenericGraphNode* RootNode, const FVector2D& Anchor)
{
	UEdNode_LTGenericGraphNode* EdNode_RootNode = EdGraph->NodeMap[RootNode];
	...
	{
		ULTGenericGraphNode* Child = RootNode->ChildrenNodes[i];
		UEdNode_LTGenericGraphNode* EdNode_ChildNode = EdGraph->NodeMap[Child];
		if (i > 0)
		{
			ULTGenericGraphNode* PreChild = RootNode->ChildrenNodes[i - 1];
			UEdNode_LTGenericGraphNode* EdNode_PreChildNode = EdGraph->NodeMap[PreChild];
...
bool ULTTreeLayoutStrategy::ResolveConflictPass(ULTGenericGraphNode* Node)
{
	bool HasConflict = false;
	for (int32 i = 0; i < Node->ChildrenNodes.Num(); ++i)
	{
		ULTGenericGraphNode* Child = Node->ChildrenNodes[i];
	...
	{
		ULTGenericGraphNode* ParentNode = Node->ParentNodes[i];
		for (int32 j = 0; j < ParentNode->ChildrenNodes.Num(); ++j)
		{
			ULTGenericGraphNode* LeftSibling = ParentNode->ChildrenNodes[j];
			if (LeftSibling == Node)
				break;
...
bool ULTTreeLayoutStrategy::ResolveConflict(ULTGenericGraphNode* LRoot, ULTGenericGraphNode* RRoot)
{
	TArray<UEdNode_LTGenericGraphNode*> RightContour, LeftContour;
	...
	if (MaxOverlapDistance > 0)
	{
		...
		TArray<ULTGenericGraphNode*> ParentNodes = RRoot->ParentNodes;
		TArray<ULTGenericGraphNode*> NextParentNodes;
...
void ULTTreeLayoutStrategy::GetLeftContour(ULTGenericGraphNode* RootNode, int32 Level, TArray<UEdNode_LTGenericGraphNode*>& Contour)
{
	UEdNode_LTGenericGraphNode* EdNode_Node = EdGraph->NodeMap[RootNode];
...
void ULTTreeLayoutStrategy::GetRightContour(ULTGenericGraphNode* RootNode, int32 Level, TArray<UEdNode_LTGenericGraphNode*>& Contour)
{
	UEdNode_LTGenericGraphNode* EdNode_Node = EdGraph->NodeMap[RootNode];
	if (Level >= Contour.Num())
	{
...
void ULTTreeLayoutStrategy::ShiftSubTree(ULTGenericGraphNode* RootNode, const FVector2D& Offset)
{
	UEdNode_LTGenericGraphNode* EdNode_Node = EdGraph->NodeMap[RootNode];
...
void ULTTreeLayoutStrategy::UpdateParentNodePosition(ULTGenericGraphNode* ParentNode)
{
	UEdNode_LTGenericGraphNode* EdNode_ParentNode = EdGraph->NodeMap[ParentNode];
	if (ParentNode->ChildrenNodes.Num() % 2 == 0)
	{
		UEdNode_LTGenericGraphNode* FirstChild = EdGraph->NodeMap[ParentNode->ChildrenNodes[0]];
		UEdNode_LTGenericGraphNode* LastChild = EdGraph->NodeMap[ParentNode->ChildrenNodes.Last()];
	...
	{
		UEdNode_LTGenericGraphNode* MidChild = EdGraph->NodeMap[ParentNode->ChildrenNodes[ParentNode->ChildrenNodes.Num() / 2]];
